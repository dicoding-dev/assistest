...
describe('create submission project test', () => {
    it('should throw error when project path not contain package.json', function () {
        ...
        expect(() => createSubmissionProject('./xxxx')).toThrow(new InvariantException('PATH_NOT_CONTAIN_PACKAGE_JSON'))
    });
    ...
    it('should throw error when package.json cannot be parsed', function () {
        ...
        expect(() => createSubmissionProject('./xxxx')).toThrow(new InvariantException('CANNOT_PARSE_PACKAGE_JSON'))
    });
    ...
    it('should throw error when package.json not contain scripts runner', function () {
        ...
        expect(() => createSubmissionProject('./xxxx')).toThrow(new InvariantException('PACKAGE_JSON_NOT_CONTAIN_SCRIPT_PROPERTY'))
    });
    ...
    it('should throw error when runner script not found', function () {
        ...
        expect(() => createSubmissionProject('.', '', 0, 'invalid-runner')).toThrow(new InvariantException('RUNNER_SCRIPT_NOT_FOUND'))
    });
